...
class Timber {
	...
	static var callbacks = new Hash<String>();
	...
	public static function processSamples(samples, amplitude : Float) {
		...
		if(wavelength > 0) {
			var noteNumber = Audio.scaleFrequency(Audio.secondsToFrequency(Audio.samplesToSeconds(wavelength)));
			...
			debug("Detected #" + (Std.int(100 * noteNumber) / 100.0)
				  + " (" + Audio.noteName(noteNumber) + "), with amplitude " + (Std.int(100 * amplitude) / 100.0));
		}
	}
